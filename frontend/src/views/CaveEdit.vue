...
<template>
  <main class="max-w-4xl mx-auto px-4 py-8 pb-20">
    <div class="flex items-center gap-2 mb-6 text-[#8b949e] text-sm">
      ...
      <span>/</span>
      <span class="text-white">{{ isNew ? 'Nouvelle cave' : 'Modifier' }}</span>
    </div>

    <div v-if="loading" class="text-center py-12 text-[#8b949e]">Chargement...</div>

    <div v-else class="space-y-6">
      <div class="bg-[#161b22] rounded-md border border-[#30363d] p-4">
        <label class="block text-xs font-medium text-[#8b949e] mb-1">Nom de la cave *</label>
        <input v-model="caveName" class="w-full bg-[#0d1117] border border-[#30363d] rounded-md p-2.5 text-white focus:border-[#58a6ff] outline-none text-sm" placeholder="Ma Cave">
      </div>

      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-medium text-white">Colonnes</h2>
          <button @click="addColumn" class="flex items-center gap-1 text-sm text-[#58a6ff] hover:text-[#388bfd] transition">
        ...
        <div v-for="(col, colIndex) in columns" :key="colIndex" class="bg-[#161b22] rounded-md border border-[#30363d] overflow-hidden">
          <div class="p-4 border-b border-[#30363d] flex items-center justify-between bg-[#0d1117]/50">
            <div class="flex items-center gap-2 flex-1">
              ...
              <input v-model="col.name" class="bg-[#0d1117] border border-[#30363d] rounded px-2 py-1 text-white text-sm focus:border-[#58a6ff] outline-none flex-1" placeholder="Nom de la colonne">
            </div>
            <button @click="removeColumn(colIndex)" class="p-1.5 text-[#8b949e] hover:text-[#f85149] transition">
          ...
          <div class="p-4">
            <div class="flex items-center justify-between mb-3">
              <span class="text-sm text-[#8b949e]">Étagères ({{ col.rows.length }})</span>
              <button @click="addRow(colIndex)" class="text-xs text-[#58a6ff] hover:underline">+ Étagère</button>
            </div>

            <div v-if="col.rows.length === 0" class="text-center py-4 text-[#8b949e] text-sm">
              Aucune étagère
            </div>

            <div v-else class="space-y-2">
              <div v-for="(row, rowIndex) in col.rows" :key="rowIndex" class="flex items-center gap-2 p-2 bg-[#0d1117] rounded border border-[#30363d]">
                <div class="flex flex-col gap-1">
                ...
                <input v-model="row.name" class="flex-1 bg-transparent border border-[#30363d] rounded px-2 py-1 text-white text-xs focus:border-[#58a6ff] outline-none" placeholder="Nom">
                <div class="flex items-center gap-1">
                  <input v-model.number="row.width" type="number" min="1" max="20" class="w-12 bg-transparent border border-[#30363d] rounded px-1 py-1 text-white text-xs text-center focus:border-[#58a6ff] outline-none" title="Largeur (positions par ligne)">
                  <span class="text-[#8b949e] text-xs">×</span>
                  <input v-model.number="row.height" type="number" min="1" max="10" class="w-12 bg-transparent border border-[#30363d] rounded px-1 py-1 text-white text-xs text-center focus:border-[#58a6ff] outline-none" title="Hauteur (lignes)">
                </div>
      ...
      <div class="flex justify-between">
        <router-link to="/caves" class="px-4 py-2 rounded-md text-[#8b949e] hover:bg-[#21262d] transition font-medium border border-[#30363d] text-sm">
          Annuler
        </router-link>
        <button @click="saveCave" :disabled="saving" class="px-6 py-2 rounded-md bg-[#238636] hover:bg-[#2ea043] text-white font-medium transition text-sm disabled:opacity-50">
